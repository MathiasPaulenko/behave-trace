.PHONY: help install dev lint lint-fix format format-check test test-cov clean build check docs docs-serve
...
clean: ## Remove build artifacts and caches
	python -c "import shutil, pathlib; [shutil.rmtree(p, ignore_errors=True) for p in ['dist','build','.pytest_cache','.mypy_cache','.ruff_cache','htmlcov'] + list(pathlib.Path('.').glob('*.egg-info'))]"
