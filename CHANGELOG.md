...
- **Attachment API** (`attach_screenshot`, `attach_dom`, `attach_text`, `log`) —
  high-level helpers for capturing screenshots, DOM snapshots, and log lines
  from `environment.py` hooks.
- **CLI** (`behave-trace`) — `show` subcommand that loads a trace file, starts
  ...
  preferred, falls back to default browser).
...
- **Unit tests** — 199 tests covering models, collector, formatter, serializer,
